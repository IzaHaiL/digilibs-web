$(document).ready(function() {
  ...
  let indeks = [];    // Inisialisasi sebagai array kosong
  let penerbit = [];    // Inisialisasi sebagai array kosong
  ...
  function fetchUserDetails() {
    $.ajax({
      ...
      error: function(xhr) {
        ...
        alert('Gagal mengambil detail pengguna. Silakan coba lagi.');
      }
    });
  }
  ...
  function fetchProjectDetails(projectId) {
    $.ajax({
      url: `${BASE_URL}/document/penelitian/id/${projectId}`,
      type: 'GET',
      headers: {
        Authorization: `Bearer ${jwt}`
      },
  ...
  function populateFormFields(data) {
    // $('#status').val(data.StatusDokumen.nama_status);
    ...
    populateIndeks(data.BelongsToManyDokumenIndeks );
    populatePenerbit(data.BelongsToManyDokumenPenerbit);
  ...
  function populateIndeks(dokumenIndeks) {
    Indeks = dokumenIndeks.map(item => ({ id: item.id })); // Initialize kategori
  
    const $field = $('#indeks');
    $field.html('<option value="" disabled selected></option>');
  
      if (dokumenIndeks && $.isArray(dokumenIndeks)) {
      $.each(dokumenIndeks, function(index, item) {
        const option = $('<option></option>').val(item.id).text(item.nama_indeks);
        $field.append(option);
      });
  
      if (dokumenIndeks.length > 0) {
        $field.val(dokumenIndeks[0].id);
      }
    } else {
      console.warn('DokumenIndeks data is not an array or is missing:', dokumenIndeks);
    }
    ...
    $.get(`${BASE_URL}/admin/document-config/indeks`, function(data) {
      ...
      $field.find('option').each(function() {
        ...
        if (!selectedValues.has(indeks.id)) {
          const option = $('<option></option>').val(indeks.id).text(indeks.nama_indeks);
          $field.append(option);
        }
      });
    });
  ...
  function populatePenerbit(dokumenPenerbit) {
    penerbit = dokumenPenerbit.map(item => ({ id: item.id })); // Initialize kategori
  
    const $field = $('#penerbit');
    $field.html('<option value="" disabled selected></option>');
  
      if (dokumenPenerbit && $.isArray(dokumenPenerbit)) {
      $.each(dokumenPenerbit, function(index, item) {
        const option = $('<option></option>').val(item.id).text(item.nama_penerbit);
        $field.append(option);
      });
  
      if (dokumenPenerbit.length > 0) {
          $field.val(dokumenPenerbit[0].id);
      }
    } else {
      console.warn('DokumenPenerbit data is not an array or is missing:', dokumenPenerbit);
    }
    ...
    $.get(`${BASE_URL}/admin/document-config/penerbit`, function(data) {
      ...
      $field.find('option').each(function() {
        ...
        data.forEach(function(penerbit) {
            if (!selectedValues.has(penerbit.id)) {
          const option = $('<option></option>').val(penerbit.id).text(penerbit.nama_penerbit);
          $field.append(option);
        }
      });
    });
...
function renderFileList() {
    ...
    const fileItem = $(`
      <div class="file-item">
        ...
        <div class="actions">
          ...
          <i class="fa fa-times remove-file" data-index="${index}" ${file.id ? `data-id="${file.id}"` : ''}></i>
        </div>
      </div>
    `);
  ...
  function getFormData() {
    ...
    formData.append('judul_inggris', $('#judul_inggris').val());
    formData.append('abstrak[konten]', $('#konten').val());
    formData.append('abstrak[konten_inggris]', $('#konten_inggris').val());
    formData.append('url', $('#url').val());

    const kontributor = $('#namaKontributor').val();
    if (kontributor) {
      formData.append('kontributor[0][id]', kontributor);
    }
    $('.dynamic-select').each(function(index) {
      const contributorId = $(this).val();
      if (contributorId) {
        formData.append(`kontributor[${index + 1}][id]`, contributorId);
      }
    });

    $('#kategorifield option:selected').each(function(index) {
      formData.append(`kategori[${index}][id]`, $(this).val());
    });

    $('#indeks option:selected').each(function(index) {
      formData.append(`indeks[${index}][id]`, $(this).val());
    });

    $('#penerbit option:selected').each(function(index) {
      formData.append(`penerbit[${index}][id]`, $(this).val());
    });


    filesArray.forEach(file => formData.append('files', file));
    if (deleteIdArray.length > 0) {
      formData.append('deleteIdArray', deleteIdArray);
    }
  ...
  async function updateProjectDetails() {
    ...
    try {
      const response = await fetch(`${BASE_URL}/document/penelitian/${id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${jwt}`
        },

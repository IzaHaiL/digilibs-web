document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function fetchFinalProjects(selectedYear) {
      const finalProjectsResponse = await fetch(`http://localhost:3000/finalprojects/private/?page=1&pageSize=99999&year=${selectedYear}`, {
        headers: {
    ...
    async function fetchResearch(selectedYear) {
      const researchResponse = await fetch(`http://localhost:3000/researchs/private/?page=1&pageSize=99999&year=${selectedYear}`, {
        headers: {
          'Authorization': `Bearer ${jwt}`
        }
      });

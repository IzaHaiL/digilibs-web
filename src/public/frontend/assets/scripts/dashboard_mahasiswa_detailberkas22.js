...
function renderFileList() {
  ...
  function showPDFInModal(url) {
    ...
    loadingTask.promise.then(pdf => {
      ...
      modalBody.innerHTML = '<p>Failed to load PDF.</p>';
    });
  ...
  async function updateProjectDetails() {
    ...
    try {
      const response = await fetch(`${BASE_URL}/document/tugas-akhir/${id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${jwt}`
        },
  ...
  $('form').on('submit', function(event) {
    ...
    if (checkForChanges()) {
      ...
      const modalHtml = `
        <div classass="modal fade" id="noChangesModal" tabindex="-1" aria-labelledby="noChangesModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">

...
$(document).ready(function() {
  $('#updateProfileForm').on('submit', function(event) {
    ...
    $.ajax({
      url: `${BASE_URL}/users/update/mahasiswa/${userData.data.user_id}`,
      type: 'PUT',
...
$(document).ready(function() {
  ...
  $.ajax({
    ...
    success: function(item) {
      ...
      $('#email').val(item.MahasiswaUsers.email);
      $('#fullname').val(`${item.MahasiswaUsers.UsersDetails.fullName}`);
      $('#nim').val(item.nim);
      $('#nik').val(item.MahasiswaUsers.UsersDetails.nik);
      $('#address').val(item.MahasiswaUsers.UsersDetails.address);
      $('#placeOfBirth').val(item.MahasiswaUsers.UsersDetails.placeOfBirth);
      $('#dateOfBirth').val(item.MahasiswaUsers.UsersDetails.dateOfBirth);
      $('#gender').val(item.MahasiswaUsers.UsersDetails.gender ? 'male' : 'female');
      $('#pp-profile').attr('src', item.MahasiswaUsers.UsersDetails.profilePicture);
      $('#facultySearch').val(item.MahasiswaProdi.ProdiFakultas.nama);
      $('#studyProgramSearch').val(item.MahasiswaProdi.nama);
      $('#phone').val(item.MahasiswaUsers.UsersDetails.phoneNumber);
    },
    error: function(xhr, status, error) {

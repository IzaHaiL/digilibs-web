  ...
  async function fetchData(jwt, page, pageSize) {
    try {
      const response = await fetch(
          `${BASE_URL}/document/penelitian/prodi?page=${page}&pageSize=${pageSize}`,
        {
          headers: {
...
async function viewDetails (id) {
  ...
  window.location.href = `/dashboard/prodi/detail/penelitian?id=${id}`
...
function updateStatusCounts() {
  const jwt = getJwtFromCookies() // Get JWT from cookies
  $.ajax({
    url: `${BASE_URL}/document/status/prodi/count/Penelitian`,
    type: 'GET',
    headers: {
      Authorization: `Bearer ${jwt}`
    },

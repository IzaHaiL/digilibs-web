  ...
  async function fetchData(jwt, page, pageSize) {
    try {
      const response = await fetch(
        `${BASE_URL}/document/tugas-akhir/prodi?page=${page}&pageSize=${pageSize}`,
        {
          headers: {
...
async function viewDetails (id) {
  ...
  window.location.href = `/dashboard/prodi/detail/tugasakhir?id=${id}`
...
function updateStatusCounts() {
  const jwt = getJwtFromCookies() // Get JWT from cookies
  $.ajax({
    url: `${BASE_URL}/document/status/prodi/count/Tugas Akhir`,
    type: 'GET',
    headers: {
      Authorization: `Bearer ${jwt}`
    },

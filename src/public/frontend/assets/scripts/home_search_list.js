...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const response = await fetch(`${BASE_URL}/finalprojects/public/search?q=${searchQuery}`);
    if (!response.ok) {
    ...
    function updateTable(data) {
      ...
      if (data.total_count === 0) {
        ...
        const itemsPerPage = data.per_page || 5; // Default to 5 items per page if per_page is not provided
        ...
        data.data.forEach((item, index) => {
          ...
          titleLink.href = `/home/detail?id=${item.research_id || item.project_id}`;
    ...
    async function fetchPage(page) {
      try {
        const response = await fetch(`${BASE_URL}/finalprojects/public/search?q=${searchQuery}&page=${page}`);
        if (!response.ok) {
          throw new Error('Failed to fetch data.');
        }
